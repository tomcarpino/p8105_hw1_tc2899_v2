...
The size of the dataset is `r dim(problem2b)`. The mean of x is `r mean(problem2$x)` and the median of x is `r median(problem2$x)`. The proportion of cases for which the logical vector is TRUE is `r mean(problem2$logical)`. 
...
```{r xy_scattter1}
ggplot(problem2b, aes(x = problem2$x, y = problem2$y, color = problem2$y)) + geom_point()

ggsave("problem2b.pdf")

ggplot(problem2b, aes(x = problem2$x, y = numeric, color = numeric)) + geom_point()

ggplot(problem2b, aes(x = problem2$x, y = factor, color = factor)) + geom_point()
```
